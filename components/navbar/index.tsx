...
export default function Navbar() {
	const navbarItems = navbarItemsData.items.map((item) => {
		return (
			<div key={item.name} className="cursor-pointer px-4 hover:bg-neutral-700 border-b-2 border-transparent hover:border-slate-300 block py-5">
				<Link href={item.link}>
					<span className="font-serif font-light text-slate-50 text-sm">{item.name}</span>
				</Link>
			</div>
		);
	});
